...
function renderHomeAssistant(query = "", matches = []) {
...
function bindHomeQuestionButtons() {
  document.querySelectorAll("[data-home-question]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      renderHomeAssistant(question, findHomeAssistantMatches(question));
    });
  });
}
...
function setupSearch() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    renderHomeAssistant(query, findHomeAssistantMatches(query));
  });
}
