(() => {
  ...
  function markerSize(m) { return Math.max(7, Math.min(24, validNumber(m) ? 6 + m * 2.4 : 7)); }
  ...
  async function loadQuakes() {
    ...
    try {
      ...
      state.all = Array.isArray(data.features)
        ? data.features.map(parseQuake).filter(q => validNumber(q.lat) && validNumber(q.lon))
        : [];
      ...
      setStatus(
        state.all.length
          ? `${state.all.length} événement(s) réel(s) récupéré(s) depuis l’USGS.`
          : 'Aucune donnée disponible actuellement.',
        state.all.length ? 'live' : 'neutral'
      );
    } catch (e) {
      state.all = [];
      state.filtered = [];
  ...
  function applyFilters(skipRender = false) {
    ...
    state.filtered = state.all.filter(q =>
      (!min || (validNumber(q.mag) && q.mag >= min)) &&
      (!query || String(q.place || '').toLowerCase().includes(query))
    );
  ...
  function popup(q) {
    return `<strong>Magnitude ${esc(validNumber(q.mag) ? q.mag.toFixed(1) : 'non disponible')}</strong><br>` +
      `📍 ${esc(q.place || 'Localisation non disponible')}<br>` +
      `🕐 ${q.time ? esc(fmtDate.format(new Date(q.time))) : 'Date indisponible'}<br>` +
      `🌐 Profondeur: ${validNumber(q.depth) ? esc(q.depth.toFixed(1)) + ' km' : 'indisponible'}<br>` +
      `Coordonnées: ${esc(q.lat?.toFixed?.(3) || '--')}, ${esc(q.lon?.toFixed?.(3) || '--')}<br>` +
      `<a href="${esc(q.url)}" target="_blank" rel="noopener noreferrer">Source officielle USGS</a>`;
  }

  function invalidateMapSize(delay = 50) {
    if (!state.map) return;
  ...
  function initMap() {
    ...
    if ('ResizeObserver' in window) {
      const observer = new ResizeObserver(() => invalidateMapSize(80));
      observer.observe(container);
    }

    window.addEventListener('resize', () => invalidateMapSize(100), { passive: true });
    window.addEventListener('orientationchange', () => invalidateMapSize(150), { passive: true });
  ...
  function renderMap() {
    ...
    // Canvas + limite d’affichage : la carte reste fluide même avec beaucoup de séismes.
    ...
    visible.forEach(q => {
      const radius = markerSize(q.mag);
      L.circleMarker([q.lat, q.lon], {
        renderer: state.renderer,
        radius,
        className: `quake-marker ${magClass(q.mag)}`,
        color: '#ffffff',
        weight: 1,
        fillOpacity: 0.72,
      }).bindPopup(popup(q), { maxWidth: 320 }).addTo(state.layer);
    });
  ...
  function renderImportant() {
    ...
    const important = state.filtered
      .filter(q => validNumber(q.mag) && q.mag >= 5)
      .sort((a, b) => b.mag - a.mag)
      .slice(0, 3);

    box.innerHTML = important.length
      ? important.map(q => `<article class="quake-alert"><h3>🚨 SÉISME IMPORTANT</h3><p>Magnitude : <strong>${esc(q.mag.toFixed(1))}</strong></p><p>Région : ${esc(q.place || 'Non disponible')}</p><p>Heure : ${q.time ? esc(fmtDate.format(new Date(q.time))) : 'Non disponible'}</p>${q.tsunami === 1 ? '<p>Information tsunami signalée par le champ officiel USGS.</p>' : ''}<a href="${esc(q.url)}" target="_blank" rel="noopener noreferrer">Vérifier sur USGS</a></article>`).join('')
      : '<p class="empty-state">Aucun événement M5+ dans les résultats actuels.</p>';
  }

  function renderList() {
    const body = $('quake-list');
    body.innerHTML = state.filtered.length
      ? state.filtered.slice(0, 60).map(q => `<tr><td data-label="Magnitude">${validNumber(q.mag) ? 'M ' + esc(q.mag.toFixed(1)) : 'Non disponible'}</td><td data-label="Localisation"><a href="${esc(q.url)}" target="_blank" rel="noopener noreferrer">${esc(q.place || 'Non disponible')}</a></td><td data-label="Profondeur">${validNumber(q.depth) ? esc(q.depth.toFixed(1)) + ' km' : 'Non disponible'}</td><td data-label="Date/heure">${q.time ? esc(fmtDate.format(new Date(q.time))) : 'Non disponible'}</td></tr>`).join('')
  ...
  function renderAll() {
    ...
    renderImportant();
    renderList();
    $('quake-count').textContent = `${state.filtered.length} résultat(s) affiché(s)`;
  }

  function renderEclipses() {
    const now = Date.now();
    const future = ECLIPSES
      .filter(e => new Date(e.dateTime).getTime() > now)
      .sort((a, b) => new Date(a.dateTime) - new Date(b.dateTime));

    const next = future[0];
    const list = $('eclipse-list');

    if (!next) {
      $('next-eclipse').innerHTML = '<p>Données temporairement indisponibles.</p>';
      return;
    }
    ...
    const solar = future.find(e => e.family === 'solar');
    const lunar = future.find(e => e.family === 'lunar');
    ...
    const updateCountdown = () => {
      const diff = new Date(next.dateTime).getTime() - Date.now();
      if (diff <= 0) return renderEclipses();
      const d = Math.floor(diff / 86400000);
      const h = Math.floor((diff % 86400000) / 3600000);
      const m = Math.floor((diff % 3600000) / 60000);
      const s = Math.floor((diff % 60000) / 1000);
      const el = $('countdown');
      if (el) el.textContent = `Compte à rebours réel : ${d}j ${h}h ${m}m ${s}s`;
    };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const refresh = $('refresh-quakes');

    period.addEventListener('change', loadQuakes);
    magnitude.addEventListener('change', applyFilters);
    ...
    refresh.addEventListener('click', loadQuakes);

    // Laisser le navigateur terminer la mise en page avant d'initialiser Leaflet.
    ...
    state.refreshTimer = window.setInterval(loadQuakes, 10 * 60 * 1000);
  });
})();
